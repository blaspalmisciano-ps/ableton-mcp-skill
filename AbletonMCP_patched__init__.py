...
class AbletonMCP(ControlSurface):
                ...
                def main_thread_task():
                    try:
                        ...
                        if command_type == "create_midi_track":
                            ...
                            track = song.tracks[track_index]
                            track.arm = arm
                            result = {"track": str(track.name), "armed": arm}
                            ...
                            for p in device.parameters:
                                pd = {"name": str(p.name), "value": float(p.value),
                                      "min": float(p.min), "max": float(p.max)}
                                try:
                                    pd["default"] = float(p.default_value)
                                except:
                                    pass
                                params_list.append(pd)
                            result = {"device": str(device.name), "parameters": params_list}
                            ...
                            notes_list = []
                            try:
                                notes = clip.get_notes_extended(from_time=0, from_pitch=0, time_span=clip.length, pitch_span=128)
                                for n in notes:
                                    notes_list.append({"pitch": n.pitch, "start": n.start_time, "dur": n.duration, "vel": n.velocity})
                            except Exception:
                                try:
                                    clip.select_all_notes()
                                    notes = clip.get_selected_notes()
                                    for n in notes:
                                        notes_list.append({"pitch": n[0], "start": n[1], "dur": n[2], "vel": n[3]})
                                    clip.deselect_all_notes()
                                except Exception as e2:
                                    self.log_message("get_clip_notes error: " + str(e2))
                            result = {"note_count": len(notes_list), "notes": notes_list[:50]}
                            ...
                            target_param = None
                            for p in device.parameters:
                                if str(p.name).lower() == param_name.lower():
                                    target_param = p
                                    break
                            if target_param is None:
                                for p in device.parameters:
                                    if param_name.lower() in str(p.name).lower():
                                        target_param = p
                                        break
                            if target_param:
                                target_param.value = max(target_param.min, min(target_param.max, value))
                                result = {"param": str(target_param.name), "value": float(target_param.value)}
                            else:
                                raise Exception("Parameter '" + param_name + "' not found")
                            ...
                            if track_index < 0 or track_index >= len(song.tracks):
                                raise Exception("Track index " + str(track_index) + " out of range (0-" + str(len(song.tracks) - 1) + ")")
                            ...
                            current_type = str(track.input_routing_type.display_name) if track.input_routing_type else "None"
                            current_channel = str(track.input_routing_channel.display_name) if track.input_routing_channel else "None"
                            result = {
                            ...
                            song = self.song()
                            track = song.tracks[track_index]
                            # Set input type first if specified
                            if input_type is not None:
                                for t in track.available_input_routing_types:
                                    if input_type.lower() in str(t.display_name).lower():
                                        ...
                                        break
                            # Then set channel — exact match first, then fallback
                            ...
                            if target is None:
                                for ch in available:
                                    if ch_str in str(ch.display_name):
                                        target = ch
                                        break
                            if target is None and len(available) >= input_channel:
                                target = available[input_channel - 1]
                            if target:
                                track.input_routing_channel = target
                                result = {"type": str(track.input_routing_type.display_name), "channel": str(target.display_name)}
                            else:
                                raise Exception("Channel " + str(input_channel) + " not found")
    ...
    def _add_notes_to_clip(self, track_index, clip_index, notes):
        ...
        try:
            ...
            # Add notes using the modern API (Live 11.1+)
            try:
                import Live
                note_specs = []
                for n in live_notes:
                ...
                self.log_message("Added {0} notes via add_new_notes".format(len(note_specs)))
            except Exception as e1:
                self.log_message("add_new_notes failed: {0}, trying replace_selected_notes".format(str(e1)))
                # Fallback: old API
                try:
                    clip.select_all_notes()
                    clip.replace_selected_notes(tuple(live_notes))
                    self.log_message("Added {0} notes via replace_selected_notes".format(len(live_notes)))
                except Exception as e2:
                    self.log_message("replace_selected_notes failed: {0}, trying set_notes".format(str(e2)))
                    clip.set_notes(tuple(live_notes))

            # Force UI refresh — toggle loop to make Ableton redraw the clip
            try:
    ...
    def _do_create_audio_track(self, index, response):
        try:
            song = self.song()
            if index == -1:
                index = len(song.tracks)
            ...
            response["result"] = {"name": str(track.name), "index": index}
        except Exception as e:
            response["status"] = "error"
            response["message"] = str(e)

    def _do_set_track_input_routing(self, track_index, input_channel, response):
        try:
            song = self.song()
            track = song.tracks[track_index]
            available = track.available_input_routing_channels
            target = None
            for ch in available:
                if str(input_channel) in str(ch.display_name):
                    target = ch
                    break
            if target is None and len(available) >= input_channel:
                target = available[input_channel - 1]
            if target:
                track.input_routing_channel = target
                response["status"] = "success"
                response["result"] = {"message": f"Input set to: {target.display_name}"}
            else:
                response["status"] = "error"
                response["message"] = f"Channel {input_channel} not found"
        except Exception as e:
            response["status"] = "error"
            response["message"] = str(e)
